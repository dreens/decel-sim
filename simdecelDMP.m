...
%MATLAB Simulation of OH experiment
    ...
    ri.dname = 'NewDecelPacketsEndBetweenRods2';
    ri.num = 1e5;
    ...
    ri.dist = 'flat';
    ri.continue = false;
    %phaserange = 50:2:80; iii=1;
    %ri.contname = cell(1,length(phaserange));
    %ri.contfillstub = 'VSF';%{'norm','SF','VSF'};
    %for n=phaserange
    %    ri.contname{iii} = sprintf('B%%s56p%d',n); iii = iii + 1;
    %end
    ...
    ri.decels{1} = struct('a','longdecel','b','ppmm_2mm');
    ...
    ri.phase = num2cell(56.54:.02:56.84);
    ri.phi2off = 0;
    n = 333;
    %ri.chargetype{1} = 'lt';
    ...
    save('Partials/endBetweenLast4X.mat','rsf')
